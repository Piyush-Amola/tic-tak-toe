...
const resetGame = () => {
  ...
  main.style.display = "block"; // Show the game board again
  turnDisplay.innerText = "Player 1's Turn (O)";
};
...
boxes.forEach((box) => {
  box.addEventListener("click", () => {
    if (box.innerText === "") {
      ...
      if (turnO) {
        box.innerText = "O";
        turnDisplay.innerText = "Player 2's Turn (X)";
        turnO = false;
      } else {
        box.innerText = "X";
        turnDisplay.innerText = "Player 1's Turn (O)";
...
const gameDraw = () => {
  ...
  main.style.display = "none"; // Hide the game board
};
